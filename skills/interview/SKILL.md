...
Write `interview/<slug>/synthesis.html` per `references/html-template.md`, **in the chosen language**, structured in three parts at both depths — **Part A carries the three deliverables** (the whole picture, the contract, the acceptance criteria), Part B the evidence behind them, Part C what is still open and what you propose. Deep fills all three fully; quick compresses B and promotes C1. Required:
- **Fully self-contained**: inline CSS, diagrams as **inline SVG** (no CDN, no mermaid — opens offline, publishable as an Artifact).
- **Part A comes first and is the point of the document**: the whole picture (ASCII *and* SVG), the process contract with its chain check, and the acceptance criteria. Someone who reads only Part A should be able to act; B and C exist so they can check A rather than trust it.
- **Each of the three ships as a copy-pasteable plain-text block.** They are what gets handed to whoever builds the thing, and an artifact that cannot be lifted off the page gets retyped and mangled.
